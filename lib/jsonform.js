...
jsonform.elementTypes = {
  ...
  'array': {
    ...
    '<a href="#" class="_jsonform-array-item-copy"><i class="icon-plus-sign" title="Copy item"></i></a> ' +
    ...
    'onInsert': function (evt, node) {
      ...
      var copyItem = function (idx, currentIndex) {
        if (boundaries.maxItems >= 0) {
          if (node.children.length > boundaries.maxItems - 2) {
            $nodeid.find('> span > a._jsonform-array-addmore')
              .addClass('disabled');
        ...
        node.copyArrayItem(idx, currentIndex, $('> ul', $nodeid).get(0));
        if ((boundaries.minItems <= 0) ||
            ((boundaries.minItems > 0) &&
              (node.children.length > boundaries.minItems - 1))) {
          $nodeid.find('> span > a._jsonform-array-deletelast')
  ...
  deleteItem(idx);
      ...
      // $(document).on('click', '._jsonform-array-item-copy', function (evt) {
      $nodeid.on('click', '._jsonform-array-item-copy', function (evt) {
        evt.preventDefault();
        evt.stopPropagation();
        var idx = node.children.length;
        var currentIndex = $(this).parent().data('idx');
        copyItem(idx, currentIndex);
      });
 ...
 *
 * @function
 * @param {Number} currentIndex The index number of the item to copy
 */

formNode.prototype.copyArrayItem = function (idx, currentIndex, domElement) {
  var i = 0;
  // Insert element at the end of the array if index is not given
  if (idx === undefined) {
    idx = this.children.length;
  ...
  // Copy values into the child
  var value = this.children[currentIndex].getFormValues(this.children[idx].arrayPath);
  child.computeInitialValues(value, true);
  ...
  // Initialize the blank node we've created with default values
  // this.children[idx].resetValues();
  // this.children[idx].computeInitialValues(value, true);
